...
from .worker import Workers, locked_directory
...
def process_gcov_data(
    ...
    if filtered:
        LOGGER.info(f"  Filtering coverage data for file {fname}")
        return

    if excluded:
        LOGGER.info(f"  Excluding coverage data for file {fname}")
        return
...
def select_gcov_files_from_stdout(
    ...
    for line in out.splitlines():
        ...
        if filtered:
            LOGGER.info(f"Filtering gcov file {fname}")
            continue

        if excluded:
            LOGGER.info(f"Excluding gcov file {fname}")
            continue
